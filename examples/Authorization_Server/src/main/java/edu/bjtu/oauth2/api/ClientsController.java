package edu.bjtu.oauth2.api;

import edu.bjtu.oauth2.configuration.AuthorityPropertyEditor;
import edu.bjtu.oauth2.configuration.SplitCollectionEditor;
...
public class ClientsController {
    @Autowired
    private JdbcClientDetailsService clientsDetailsService1;
    @InitBinder
    ...
    public String showEditForm(@RequestParam(value="client",required=false)String clientId, Model model){
        ...
        if(clientId !=null){
            clientDetails=clientsDetailsService1.loadClientByClientId(clientId);
        }
        else{
    ...
    public String editClient(
        ...
        if (newClient == null) {
            ...
            clientsDetailsService1.updateClientDetails(clientDetails);
        } else {
            clientsDetailsService1.addClientDetails(clientDetails);
        }


        if (!clientDetails.getClientSecret().isEmpty()) {
            clientsDetailsService1.updateClientSecret(clientDetails.getClientId(), clientDetails.getClientSecret());
        }
        return "redirect:/";
    }
    ...
    public String deleteClient(@ModelAttribute BaseClientDetails ClientDetails,@PathVariable("client.clientId") String id){
        clientsDetailsService1.removeClientDetails(clientsDetailsService1.loadClientByClientId(id).toString());
        return "redirect:/";
    }
}
